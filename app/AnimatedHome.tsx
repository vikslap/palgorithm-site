...
// We add 'posts' to the destructured props here
export default function AnimatedHome({ data, posts }: { data: any, posts: any[] }) {
  return (
    <main className="flex min-h-screen flex-col items-center bg-[#fdfdfc] px-6 text-zinc-900 selection:bg-zinc-200">
      ...
      {/* Hero Section - Keeps the clean Wired look */}
      ...
      {/* Blog Archive Section - Styled like an editorial column */}
      <section className="w-full max-w-2xl pb-32">
        <div className="border-t border-zinc-100 pt-16">
            ...
            {posts?.map((post: any) => (
              ...
              >
                <div className="flex flex-col space-y-4">
                  <h3 className="font-serif text-3xl md:text-4xl italic text-zinc-800 group-hover:text-indigo-600 transition-colors duration-500">
                    {post.title}
                  </h3>
                  ...
                  <div className="flex items-center pt-2">
                    <span className="text-[10px] font-bold tracking-widest text-zinc-400 uppercase group-hover:text-zinc-900 transition-colors">
                      Read Entry
                    </span>
                    <span className="ml-2 text-zinc-300 transform group-hover:translate-x-1 transition-transform">→</span>
                  </div>
                </div>
              </motion.article>
            ))}
          ...
          {(!posts || posts.length === 0) && (
            <p className="font-serif italic text-zinc-300">New entries appearing soon...</p>
          )}
        </div>
      </section>
    </main>
